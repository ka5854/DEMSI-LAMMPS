...
*/
  ...
  } else if(int(timeIntegrationFlag) == 1) { compute_rate_explicit0(evflag);
...
//    j &= NEIGHMASK;
  ...
  double dvx, dvy, dvz, dvmag, dMach;
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    itype = atom->type[i];
    if (itype != 1) continue;
    if (m0t[i] > 0.) {
      dvx = mvx[i]/m0t[i] -     v[i][0];
      dvy = mvy[i]/m0t[i] -     v[i][1];
      dvz = mvz[i]/mIt[i] - omega[i][2];
      radi = radius[i];
      dvmag = dvx*dvx + dvy*dvy + (radi*dvz)*(radi*dvz); // |dv|^2
      dMach = sqrt(dvmag)/elastic_wavespeed;
    } else {
      dvx = dvy = dvz = dMach = 0.;
    }
    v[i][2] = dMach; // diagnostic
    if ( bulkCFL >= 1. and dMach > 0.5 ) {
          v[i][0] += 0.5*dvx;
          v[i][1] += 0.5*dvy;
      omega[i][2] += 0.5*dvz;
    }
